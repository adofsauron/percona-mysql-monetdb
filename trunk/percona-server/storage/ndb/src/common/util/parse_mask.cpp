...
int
ParseThreadConfiguration::read_params(ParamValue values[],
...
int
ParseThreadConfiguration::find_next()
{
  skipblank();
  ...
  m_err_msg.assfmt("Invalid format near: '%.*s'",
                   (len > 10) ? 10 : len, m_curr_str);
  return -1;
}

unsigned int
ParseThreadConfiguration::find_type()
{
  skipblank();
...
int
ParseThreadConfiguration::find_params(char **start,
                                      char **end)
{
  skipblank();
  do
...
int
ParseThreadConfiguration::parse_params(char *str,
                                       ParamValue values[])
{
  ...
  while (* m_curr_str)
  {
    ...
    if (* m_curr_str != '=')
    {
       m_err_msg.assfmt("Missing '=' after %s in '%s'",
                        m_parse_params[idx].name,
                        m_curr_str);
       return -1;
    }
    ...
    if (res == -1)
    {
      m_err_msg.assfmt("Unable to parse %s=%s",
                       m_parse_params[idx].name,
                       m_curr_str);
      return -1;
    }
    if (res == -2)
    {
      m_err_msg.assfmt("Bitmask too big %s, %s",
                       m_parse_params[idx].name,
                       m_curr_str);
      return -1;
    }
...
void
ParseThreadConfiguration::skipblank()
{
  char *str= m_curr_str;
  while (isspace(* str))
  {
...
unsigned int
ParseThreadConfiguration::get_entry_type(const char *type)
{
...
unsigned int
ParseThreadConfiguration::get_param_len()
{
  unsigned int len = 0;
  char *str = m_curr_str;
...
int
ParseThreadConfiguration::parse_string(char *dest_str)
{
  Uint32 i = 0;
  skipblank();
  while (m_curr_str[0] != ',' &&
...
int
ParseThreadConfiguration::parse_bitmask(SparseBitmask&  mask)
{
  skipblank();
