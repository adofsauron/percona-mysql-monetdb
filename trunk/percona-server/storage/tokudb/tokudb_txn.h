...
inline void make_name(char *newname, size_t newname_len, const char *tablename,
                      const char *dictname) {
  assert_always(tablename);
  assert_always(dictname);
  size_t real_size =
      snprintf(newname, newname_len, "%s-%s", tablename, dictname);
...
inline int txn_begin(DB_ENV *env, DB_TXN *parent, DB_TXN **txn, uint32_t flags,
  ...
  *txn = NULL;
  ...
  TOKUDB_TRACE_FOR_FLAGS(TOKUDB_DEBUG_TXN, "begin txn %p %p %u r=%d", parent,
                         *txn, flags, r);
  return r;
}
