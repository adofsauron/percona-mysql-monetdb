...
#define TOKUDB_DEBUG_FLAGS(_flags) (tokudb::sysvars::debug & _flags)
...
#define TOKUDB_HANDLER_TRACE(_fmt, ...)                                     \
  fprintf(stderr, "%u %p %s:%u ha_tokudb::%s " _fmt "\n",                   \
          tokudb::thread::my_tid(), this, __FILE__, __LINE__, __FUNCTION__, \
          ##__VA_ARGS__);
...
void toku_hton_assert_fail(const char * /*expr_as_string*/,
                           ...
                           int /*line*/, int /*errno*/)
    __attribute__((__visibility__("default"))) __attribute__((__noreturn__));

#define assert_always(expr)                                                \
  ((expr) ? (void)0                                                        \
          : toku_hton_assert_fail(#expr, __FUNCTION__, __FILE__, __LINE__, \
                                  errno))
