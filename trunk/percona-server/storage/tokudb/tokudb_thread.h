...
class semaphore_t {
 public:
  ...
  enum E_WAIT { E_SIGNALLED = 0, E_INTERRUPTED = 1, E_TIMEDOUT = 2 };
...
inline rwlock_t &rwlock_t::operator=(const rwlock_t &) { return *this; }
...
inline void event_t::wait(void) {
  ...
  if (_manual_reset == false) _signalled = false;
  if (_pulsed) _pulsed = false;
...
inline event_t &event_t::operator=(const event_t &) { return *this; }
...
inline int thread_t::detach(void) { return pthread_detach(_thread); }
