...
class job_manager_t {
 public:
  class job_t {
   public:
    ...
    // method that tells the job to clean up/free resources on
    ...
    // method to obtain a sting identifying the current status of
    // the job
 ...
 public:
  ...
  // if a foreground job is attempted, any currently scheduled or
  // running background jobs will be cancelled first if another
  // foreground job is already running, a new foreground job with the
  ...
  // 'callback' with the 'extra' data provided and the original
  // 'extra' data passed when the job was scheduled
  void iterate_jobs(pfn_iterate_t callback, void *extra) const;

  // lock the bjm, this prevents anyone from running, cancelling or
  // iterating jobs in the bjm.
  inline void lock();
  inline void unlock();
...
inline void job_manager_t::job_t::cancel() {
  _cancelled = true;
  if (_running) on_cancel();
  while (_running) tokudb::time::sleep_microsec(500000);
  destroy();
}
void job_manager_t::job_t::destroy() { on_destroy(); }
inline bool job_manager_t::job_t::running() const { return _running; }
inline bool job_manager_t::job_t::cancelled() const { return _cancelled; }
inline uint64_t job_manager_t::job_t::id() const { return _id; }
