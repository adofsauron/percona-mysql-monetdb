...
class TOKUDB_SHARE {
 public:
  enum share_state_t { CLOSED = 0, OPENED = 1, ERROR = 2 };
  ...
  static TOKUDB_SHARE *get_share(const char *table_name, THR_LOCK_DATA *data,
                                 bool create_new);
...
inline int TOKUDB_SHARE::use_count() const { return _use_count; }
...
inline void TOKUDB_SHARE::set_state(TOKUDB_SHARE::share_state_t state) {
  TOKUDB_SHARE_DBUG_ENTER("file[%s]:state[%s]:use_count[%d]:new_state[%s]",
                          _full_table_name.ptr(), get_state_string(_state),
                          _use_count, get_state_string(state));
...
inline void TOKUDB_SHARE::set_row_count(uint64_t rows, bool locked) {
  TOKUDB_SHARE_DBUG_ENTER("file[%s]:state[%s]:use_count[%d]:rows[%" PRIu64
                          "]:locked[%d]",
                          _full_table_name.ptr(), get_state_string(_state),
                          _use_count, rows, locked);
  ...
  if (_rows && rows == 0) _row_delta_activity = 0;
...
inline ha_rows TOKUDB_SHARE::row_count() const { return _rows; }
inline void TOKUDB_SHARE::init_cardinality_counts(uint32_t rec_per_keys,
                                                  uint64_t *rec_per_key) {
  assert_debug(_mutex.is_owned_by_me());
...
inline void TOKUDB_SHARE::update_cardinality_counts(
    uint32_t rec_per_keys, const uint64_t *rec_per_key) {
  assert_debug(_mutex.is_owned_by_me());
  // can not change number of keys live
...
inline void TOKUDB_SHARE::allow_auto_analysis(bool reset_deltas) {
  ...
  if (reset_deltas) _row_delta_activity = 0;
}
...
typedef enum { lock_read = 0, lock_write } TABLE_LOCK_TYPE;
...
class ha_tokudb : public handler {
 private:
  ...
  uint toku_pushed_idx_cond_keyno; /* The index which the above condition is
                                      for */
  ...
  uint32_t bytes_used_in_range_query_buff;  // number of bytes used in the
                                            // range query buffer
  uint32_t curr_range_query_buff_offset;    // current offset into the range
                                            // query buffer for queries to read
  ...
  // external_lock will set this true for read operations that will be closely
  ...
  //
  // count on the number of rows that gets changed, such as when write_row
  // occurs this is meant to help keep estimate on number of elements in DB
  //
  ...
  //
  bool delay_updating_ai_metadata;   // if true, don't update auto-increment
                                     // metadata until bulk load completes
  bool ai_metadata_update_required;  // if true, autoincrement metadata must
                                     // be updated
  ...
  int pack_row_in_buff(DBT *row, const uchar *record, uint index,
                       uchar *row_buff);
  int pack_row(DBT *row, const uchar *record, uint index);
  int pack_old_row_for_update(DBT *row, const uchar *record, uint index);
  ...
  uint32_t place_key_into_dbt_buff(KEY *key_info, uchar *buff,
                                   const uchar *record, bool *has_null,
                                   int key_length);
  DBT *create_dbt_key_from_key(DBT *key, KEY *key_info, uchar *buff,
                               const uchar *record, bool *has_null,
                               bool dont_pack_pk, int key_length,
                               uint8_t inf_byte);
  DBT *create_dbt_key_from_table(DBT *key, uint keynr, uchar *buff,
                                 const uchar *record, bool *has_null,
                                 int key_length = MAX_KEY_LENGTH);
  DBT *create_dbt_key_for_lookup(DBT *key, KEY *key_info, uchar *buff,
                                 const uchar *record, bool *has_null,
                                 int key_length = MAX_KEY_LENGTH);
  DBT *pack_key(DBT *key, uint keynr, uchar *buff, const uchar *key_ptr,
                uint key_length, int8_t inf_byte);
#if defined(TOKU_INCLUDE_EXTENDED_KEYS) && TOKU_INCLUDE_EXTENDED_KEYS
  DBT *pack_ext_key(DBT *key, uint keynr, uchar *buff, const uchar *key_ptr,
                    uint key_length, int8_t inf_byte);
  ...
  int open_secondary_dictionary(DB **ptr, KEY *key_info, const char *name,
                                bool is_read_only, DB_TXN *txn);
  ...
  int write_to_status(DB *db, HA_METADATA_KEY curr_key_data, void *data,
                      uint size, DB_TXN *txn);
  int remove_from_status(DB *db, HA_METADATA_KEY curr_key_data, DB_TXN *txn);

  int write_metadata(DB *db, void *key, uint key_size, void *data,
                     uint data_size, DB_TXN *txn);
  int remove_metadata(DB *db, void *key_data, uint key_size,
                      DB_TXN *transaction);

  int update_max_auto_inc(DB *db, ulonglong val);
  int remove_key_name_from_status(DB *status_block, const char *key_name,
                                  DB_TXN *txn);
  int write_key_name_to_status(DB *status_block, const char *key_name,
                               DB_TXN *txn);
  ...
  int delete_or_rename_table(const char *from_name, const char *to_name,
                             bool is_delete);
  int delete_or_rename_dictionary(const char *from_name, const char *to_name,
                                  const char *index_name, bool is_key,
                                  DB_TXN *txn, bool is_delete);
  int truncate_dictionary(uint keynr, DB_TXN *txn);
  int create_secondary_dictionary(const char *name, TABLE *form, KEY *key_info,
                                  DB_TXN *txn, KEY_AND_COL_INFO *kc_info,
                                  uint32_t keynr, bool is_hot_index,
  ...
  int is_index_unique(bool *is_unique, DB_TXN *txn, DB *db, KEY *key_info,
                      int lock_flags);
  int is_val_unique(bool *is_unique, uchar *record, KEY *key_info,
  ...
  int insert_rows_to_dictionaries_mult(DBT *pk_key, DBT *pk_val, DB_TXN *txn,
                                       THD *thd);
  void test_row_packing(uchar *record, DBT *pk_key, DBT *pk_val);
  uint32_t fill_row_mutator(uchar *buf, uint32_t *dropped_columns,
                            uint32_t num_dropped_columns, TABLE *altered_table,
                            KEY_AND_COL_INFO *altered_kc_info, uint32_t keynr,
                            bool is_add);

  // 0 <= active_index < table_share->keys || active_index == MAX_KEY
  // tokudb_active_index = active_index if active_index < table_share->keys,
  // else tokudb_active_index = primary_key = table_share->keys
 ...
 public:
  ...
  uint extra_rec_buf_length() const { return TOKUDB_HIDDEN_PRIMARY_KEY_LENGTH; }
  ...
  static void loader_add_index_err(DB *db, int i, int err, DBT *key, DBT *val,
                                   void *error_extra);
  static void loader_dup(DB *db, int i, int err, DBT *key, DBT *val,
                         void *error_extra);
  int end_bulk_insert();
 ...
 private:
  ...
  int alter_table_add_or_drop_column(TABLE *altered_table,
                                     Alter_inplace_info *ha_alter_info);
  int alter_table_expand_varchar_offsets(TABLE *altered_table,
                                         Alter_inplace_info *ha_alter_info);
  int alter_table_expand_columns(TABLE *altered_table,
                                 Alter_inplace_info *ha_alter_info);
  int alter_table_expand_one_column(TABLE *altered_table,
                                    Alter_inplace_info *ha_alter_info,
                                    int expand_field_num);
  int alter_table_expand_blobs(TABLE *altered_table,
                               Alter_inplace_info *ha_alter_info);
  void print_alter_info(TABLE *altered_table,
                        Alter_inplace_info *ha_alter_info);
 ...
 public:
 private:
  int tokudb_add_index(TABLE *table_arg, KEY *key_info, uint num_of_keys,
                       DB_TXN *txn, bool *inc_num_DBs, bool *modified_DB);
  static int tokudb_add_index_poll(void *extra, float progress);
  void restore_add_index(TABLE *table_arg, uint num_of_keys,
 ...
 public:
  ...
  int read_row_callback(uchar *buf, uint keynr, DBT const *row,
                        DBT const *found_key);
  int read_primary_key(uchar *buf, uint keynr, DBT const *row,
                       DBT const *found_key);
  int unpack_blobs(uchar *record, const uchar *from_tokudb_blob,
                   uint32_t num_blob_bytes, bool check_bitmap);
  int unpack_row(uchar *record, DBT const *row, DBT const *key, uint index);

  int prefix_cmp_dbts(uint keynr, const DBT *first_key, const DBT *second_key) {
    return tokudb_prefix_cmp_dbt_key(share->key_file[keynr], first_key,
                                     second_key);
  }
  ...
  int fill_range_query_buf(bool need_val, DBT const *key, DBT const *row,
