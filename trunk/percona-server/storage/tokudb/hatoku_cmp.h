...
//     If no primary key is defined, then an eight byte hidden primary key is
//     autogenerated (like an auto increment) and used Secondary keys contains
//     the defined key and the primary key. Two examples:
//       1) table foo (a int, b int, c int, d int, key(b))
//           The key of the main dictionary contains an eight byte autogenerated
//           hidden primary key The key of key-b is the column 'b' followed by
...
//      For the main dictionary and clustering keys, they contain all columns
//      that do not show up in the dictionary's key Two examples:
//        1) table foo (a int, b int, c int, d varchar(100), primary key(a),
//        clustering key d(d), clustering key d2(d(20))
//           the val of the main dictionary contains (b,c,d)
//           the val of d contains (b,c)
//           the val of d2 contains (b,c,d). d is there because the entire row
//           does not show up in the key
//      Vals are encoded as follows. They have four components:
//      1) Null bytes: contains a bit field that states what columns are NULL.
//      2) Fixed fields: all fixed fields are then packed together. If a fixed
//      field is NULL, its data is considered junk 3) varchars and varbinaries:
//      stored in two pieces, first all the offsets and then all the data. If a
//      var field is NULL, its data is considered junk 4) blobs: stored in
//      (length, data) pairs. If a blob is NULL, its data is considered junk An
//      example:
//        Table: (a int, b varchar(20), c blob, d bigint, e varbinary(10), f
//        largeblob, g varchar(10)) <-- no primary key defined Row inserted: (1,
//        "bbb", "cc", 100, "eeeee", "ffff", "g") The packed format of the val
//        looks like:
//          NULL byte <-- 1 byte to encode nothing is NULL
//          1 <-- four bytes for 'a'
//          100 <-- four bytes for 'd'
//          3,8,9 <--offsets for location of data fields, note offsets point to
//          where data ENDS "bbbeeeeeg" <-- data for variable length stuff
...
//
// used to define a couple of characteristics of a packed val for the main
// dictionary or a clustering dictionary fixed_field_size is the size of the
// fixed fields in the val. len_of_offsets is the size of the bytes that make up
// the offsets of variable size columns Some notes:
//   If the val has no fixed fields, fixed_field_size is 0
//   If the val has no variable fields, len_of_offsets is 0
//   The number of null bytes at the beginning of a row is not saved, it is
//   derived from table_share->null_bytes The pointer to where the variable data
//   in a val starts is table_share->null_bytes + fixed_field_size +
//   len_of_offsets To figure out where the blobs start, find the last offset
//   listed (if offsets exist)
//
...
class KEY_AND_COL_INFO {
 ...
 public:
  ...
  // because the column is stored in the key. So, for example, the table (a
  // int, b int, c int, d int, primary key (b,d)) will have bit 1 (for 'b')
  // and bit 3 (for 'd') of the primary key's bitmap set for the main
  // dictionary's bitmap, because 'b' and 'd' do not show up in the val
  //
  MY_BITMAP key_filters[MAX_KEY + 1];
  //
  // following three arrays are used to identify the types of rows in the
  // field If table->field[i] is a fixed field:
  ...
  //   length_bytes[i] stores the number of bytes MySQL uses to encode the
  //   length of the field in table->record[0] 'i' does not show up in the
  //   array blob_fields
...
static void get_var_field_info(uint32_t *field_len, uint32_t *start_offset,
                               uint32_t var_field_index,
                               const uchar *var_field_offset_ptr,
                               uint32_t num_offset_bytes);

static void get_blob_field_info(uint32_t *start_offset, uint32_t len_of_offsets,
                                const uchar *var_field_data_ptr,
                                uint32_t num_offset_bytes);

static inline uint32_t get_blob_field_len(const uchar *from_tokudb,
                                          uint32_t len_bytes) {
  uint32_t length = 0;
  switch (len_bytes) {
    case (1):
...
static inline const uchar *unpack_toku_field_blob(uchar *to_mysql,
...
static uchar *pack_toku_key_field(
    uchar *to_tokudb, uchar *from_mysql, Field *field,
...
static uchar *unpack_toku_key_field(uchar *to_mysql, uchar *from_tokudb,
                                    Field *field, uint32_t key_part_length);
...
//
// function to convert a hidden primary key into a byte stream that can be
// stored in DBT
//
...
//
// function that takes a byte stream of a hidden primary key and returns a
// ulonglong
//
static inline ulonglong hpk_char_to_num(uchar *val) { return uint8korr(val); }

static int tokudb_compare_two_keys(const void *new_key_data,
                                   const uint32_t new_key_size,
                                   const void *saved_key_data,
                                   const uint32_t saved_key_size,
                                   const void *row_desc,
                                   const uint32_t row_desc_size,
                                   bool cmp_prefix, bool *read_string);

static int tokudb_cmp_dbt_key(DB *db, const DBT *keya, const DBT *keyb);

// TODO: QQQ Only do one direction for prefix.
static int tokudb_prefix_cmp_dbt_key(DB *file, const DBT *keya,
                                     const DBT *keyb);
...
static int tokudb_compare_two_key_parts(
    ...
    const void *row_desc, const uint32_t row_desc_size, uint max_parts);

static int tokudb_cmp_dbt_key_parts(DB *file, const DBT *keya, const DBT *keyb,
                                    uint max_parts);

static int create_toku_key_descriptor(uchar *buf, bool is_first_hpk,
                                      KEY *first_key, bool is_second_hpk,
                                      KEY *second_key);
...
static uint32_t create_toku_secondary_key_pack_descriptor(
    ...
    TABLE *table, KEY_AND_COL_INFO *kc_info, KEY *key_info, KEY *prim_key);
...
static uint32_t max_key_size_from_desc(void *row_desc, uint32_t row_desc_size);

static uint32_t pack_key_from_desc(uchar *buf, void *row_desc,
                                   uint32_t row_desc_size, const DBT *pk_key,
                                   const DBT *pk_val);
