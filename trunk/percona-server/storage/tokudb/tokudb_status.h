...
int read(DB *status_db, DB_TXN *txn, HA_METADATA_KEY k, void *p, size_t s,
         size_t *sp) {
  DBT key = {};
  key.data = &k;
  key.size = sizeof(k);
...
// returns 0 if successful.
int read_realloc(DB *status_db, DB_TXN *txn, HA_METADATA_KEY k, void **pp,
                 size_t *sp) {
  DBT key = {};
  key.data = &k;
  key.size = sizeof(k);
...
// returns 0 if successful.
int write_low(DB *status_db, void *key_data, uint key_size, void *val_data,
              uint val_size, DB_TXN *txn) {
  DBT key = {};
  key.data = key_data;
  key.size = key_size;
  DBT value = {};
...
// returns 0 if successful.
int write(DB *status_db, HA_METADATA_KEY curr_key_data, void *val,
          size_t val_size, DB_TXN *txn) {
  return tokudb::metadata::write_low(status_db, &curr_key_data,
                                     sizeof(curr_key_data), val, val_size, txn);
}

// remove a key from the status dictionary.
// auto create a txn if necessary.
// returns 0 if successful.
int remove_low(DB *status_db, void *key_data, uint key_size, DB_TXN *txn) {
  DBT key = {};
  key.data = key_data;
  key.size = key_size;
...
// the key must be a HA_METADATA_KEY
// returns 0 if successful.
int remove(DB *status_db, HA_METADATA_KEY curr_key_data, DB_TXN *txn) {
  return tokudb::metadata::remove_low(status_db, &curr_key_data,
                                      sizeof(curr_key_data), txn);
}
...
int close(DB **status_db_ptr) {
  ...
  if (status_db) {
    ...
    if (error == 0) *status_db_ptr = NULL;
  }
  return error;
}

int create(DB_ENV *env, DB **status_db_ptr, const char *name, DB_TXN *txn) {
  int error;
  DB *status_db = NULL;
  ...
  if (error == 0) {
    error = status_db->open(status_db, txn, name, NULL, DB_BTREE,
                            DB_CREATE | DB_EXCL, 0);
  }
  if (error == 0) {
...
int strip_frm_data(DB_ENV *env) {
  ...
  while (1) {
    ...
    if (strstr(iname, "_status_")) {
      ...
      // GOL : this is a godawful hack. The inventors of this did
      // not think it would be a good idea to use some kind of
      // magic identifier k/v pair so that you can in fact tell a
      // proper status file from any other file that might have
      // the string _status_ in it. Out in ha_tokudb::create, when
      // the status file is initially created, it is immediately
      // populated with:
      //    uint hatoku_new_version=HA_TOKU_VERSION=4 and
      //    uint hatoku_capabilities=HA_TOKU_CAP=0
      // Since I can't count on the fact that these values
      // are/were _always_ 4 and 0, I can count on the fact that
      // they _must_ be there and the _must_ be sizeof(uint). That
      // will at least give us a much better idea that these are
      // in fact status files.
      void *p = NULL;
      size_t sz;
      r = tokudb::metadata::read_realloc(status_db, txn, hatoku_new_version, &p,
                                         &sz);
      if (r != 0) {
        fprintf(stderr,
      ...
      r = tokudb::metadata::read_realloc(status_db, txn, hatoku_capabilities,
                                         &p, &sz);
      if (r != 0) {
        fprintf(stderr,
                "TokuDB strip_frm_data : does not look like a real TokuDB "
      ...
      // OK, st this point, it is probably a status file, not 100%
