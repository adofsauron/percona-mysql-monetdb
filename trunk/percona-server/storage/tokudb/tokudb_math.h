...
static uint64_t uint_low_endpoint(TOKUDB_UNUSED(uint length_bits)) { return 0; }
...
TOKUDB_UNUSED(static uint64_t uint_add(uint64_t x, uint64_t y, uint length_bits,
                                       bool *over));
...
TOKUDB_UNUSED(static uint64_t uint_sub(uint64_t x, uint64_t y, uint length_bits,
                                       bool *over));
...
static int64_t int_sign_extend(int64_t n, uint length_bits) {
  if (n & (1ULL << (length_bits - 1))) n |= ~uint_mask(length_bits);
  return n;
}

// Add two signed ints with max maximum value.
// If there is an overflow then set the sum to the max or the min of the int
// range, depending on the sign bit. Sign extend to 64 bits. Return the sum
// and the overflow.
TOKUDB_UNUSED(static int64_t int_add(int64_t x, int64_t y, uint length_bits,
                                     bool *over));
static int64_t int_add(int64_t x, int64_t y, uint length_bits, bool *over) {
  int64_t mask = uint_mask(length_bits);
  int64_t n = (x + y) & mask;
  *over = (((n ^ x) & (n ^ y)) >> (length_bits - 1)) & 1;  // check for overflow
  if (n & (1LL << (length_bits - 1))) n |= ~mask;          // sign extend
  return n;
}
...
// range, depending on the sign bit. Sign extend to 64 bits. Return the sum
// and the overflow.
TOKUDB_UNUSED(static int64_t int_sub(int64_t x, int64_t y, uint length_bits,
                                     bool *over));
