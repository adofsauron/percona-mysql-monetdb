...
#ifndef KEYRING_KMS_BACKEND_INCLUDED
#define KEYRING_KMS_BACKEND_INCLUDED
...
namespace backend {
class Keyring_kms_backend final {
 public:
  explicit Keyring_kms_backend(const std::string &keyring_kms_name,
                               bool read_only,
  ...
  */
  ...
  void create_file_if_missing(std::string const &file_name);

 private:
  config::Config_pod config_;

  /** Keyring file */
  std::string keyring_kms_name_;
...
#endif  // !KEYRING_KMS_BACKEND_INCLUDED
